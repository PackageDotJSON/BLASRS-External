const DB_QUERIES = {
  ...
  AUTH_USER: `SELECT USER_COMPANY.COMPANY_ID, USER_COMPANY.COMPANY_NAME 
  FROM ESUSER.USER_COMPANY 
  INNER JOIN ESUSER.USER_PROFILE ON USER_COMPANY.USER_ID_FK = USER_PROFILE.USER_ID
  WHERE USER_COMPANY.USER_ID_FK = ? AND 
  USER_COMPANY.COMPANY_ID = (
    SELECT CMPNYID
    FROM ICM_COMPANY
    WHERE ICM_COMPANY.CMPNYINCNO = ?
  ) AND 
  DECRYPT_CHAR(USER_PROFILE.PASSWORD, ?) = ?`,
  AUTH_PIN_CODE: `SELECT * FROM USER_PIN_HISTORY WHERE USER_ID_FK = ? AND USER_PIN = ?`,
  ...
  COMPANY_INCNO, SUBMITTED_BY, UPLOAD_ID, RECORD_COUNT, PERIOD_ENDED, UPLOAD_FILE) VALUES (BLASRS_DATA_SUBMITTER_SEQ.nextval, 
  :userId, :userPin, :recordType, :companyId, :companyName, :companyIncno, :submittedBy, BLASRS_DATA_SUBMITTER_SEQ.nextval, 
  ...
  (BLASRS_DATA_SEQ.nextval, :userId, :companyId, :companyName, :companyIncno, 'Excel', :createdBy, 'Submitted', 
  ...
  GET_FILE: `SELECT UPLOAD_FILE FROM CRS.BLASRS_DATA_SUBMITTER WHERE UPLOAD_ID = :uploadId`,
